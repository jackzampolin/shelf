...
func (j *Job) HandleStructureClassifyComplete(ctx context.Context, result jobs.WorkResult, info WorkUnitInfo) ([]jobs.WorkUnit, error) {
	...
	// Chapter boundaries are scan-page based. When multiple ToC entries begin
	// on the same scan page, each extracted chapter contains that entire page.
	// Letting more than one of those entries into audio (or downstream parsed
	// corpora) repeats the page verbatim. Keep the last entry that the model
	...
	if coalesced > 0 {
		...
		if logger != nil {
			logger.Info("coalesced same-page chapter audio",
				"book_id", j.Book.BookID,
				"chapters_excluded", coalesced)
		}
	}
