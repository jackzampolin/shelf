...
func loadPageImageTool() providers.Tool {
	return providers.Tool{
		...
		Function: providers.ToolFunction{
			...
			Description: "Load a page image to see it visually. Use for visual verification when OCR is unclear (especially Roman numerals) or when you need to confirm the heading format. WORKFLOW: Document what you see on the current page before loading the next one.",
			Parameters: mustMarshal(map[string]any{
				...
				"properties": map[string]any{
					...
					"current_page_observations": map[string]any{
						"type":        "string",
						"description": "What did you see on the currently loaded page? Required if a page is already loaded. Document: Is this the chapter start? What heading do you see?",
					},
				},
...
func (t *ChapterFinderTools) loadPageImage(ctx context.Context, pageNum int, currentPageObservations string) (string, error) {
	...
	if t.isInExcludedRange(pageNum) {
		messageParts = append(messageParts,
			"⚠️ This page is in an EXCLUDED range (back matter).")
	}
