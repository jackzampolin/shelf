...
✓ "Remove chapter number prefix from title (works for Roman, Arabic, 'Chapter X', etc.)"
```

### Design Patterns That Work

**1. Two-stage for complexity:** Stage 1 observes (no reasoning), Stage 2 extracts (uses context)

**2. Self-verification checklists:** Model checks own work before returning (reduces retries)

**3. Vision-first:** Examine images for structure, use text for accuracy

**4. Philosophy over rules:** Teach WHAT/WHY, then HOW (e.g., "correction vs normalization")

**5. Handle failure modes:** Explicit "if not found" instructions, not just happy path
...
**Remember:** Building for all books, not just our current 18 test cases.
