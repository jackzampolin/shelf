"""Prompts for Extract: Single-call complete ToC extraction"""

SYSTEM_PROMPT = """You are a Table of Contents extraction specialist. You will be given the complete Table of Contents from a book (2-6 pages), and you must extract ALL entries as a structured list.
...
def build_user_prompt(
    toc_pages: list,
    structure_summary: dict = None
) -> str:
    """
    Build prompt for single-call complete ToC extraction.

    Args:
        toc_pages: List of dicts with {page_num, ocr_text}
        structure_summary: Optional global structure from find phase
    ...
    for page in toc_pages:
        complete_text += f"\n{'='*80}\n"
        complete_text += f"PAGE {page['page_num']} of {len(toc_pages)}\n"
        complete_text += f"{'='*80}\n"
        complete_text += page['ocr_text']
        complete_text += "\n"

    prompt = f"""<task>
Extract ALL Table of Contents entries from these {len(toc_pages)} pages.
...
Total levels: {structure_summary.get('total_levels', 'unknown')}
"""
