...
def process_single_page(
    ...
    """Process a single page with Mistral OCR (single attempt - retries handled at queue level)."""
    ...
    if not page_file.exists():
        ...
        return {"success": False, "page_num": page_num, "error": error_msg, "retryable": False}
    ...
    try:
        ...
        if not ocr_response.pages or len(ocr_response.pages) == 0:
            ...
            return {"success": False, "page_num": page_num, "error": error_msg, "retryable": True}
        ...
        error_msg = str(e)
        # Most API errors are retryable (network, rate limit, etc)
        retryable = True
        return {"success": False, "page_num": page_num, "error": error_msg, "retryable": retryable}
...
def process_batch(
    ...
    """Process batch of pages with Mistral OCR using queue-based retry."""
    ...
    # Work queue and retry tracking
    work_queue = Queue()
    retry_counts = {}  # page_num -> retry_count
    retry_lock = Lock()

    # Initialize queue with all pages
    for page_num in remaining_pages:
        work_queue.put(page_num)
        retry_counts[page_num] = 0
    ...
    def worker():
        """Worker pulls pages from queue, processes them, re-queues failures."""
        nonlocal pages_processed, total_cost, total_chars, total_images, total_time

        while True:
            try:
                page_num = work_queue.get(timeout=1)
            except:
                # Queue empty and timeout - worker exits
                break

            # Check retry count
            with retry_lock:
                current_retry = retry_counts.get(page_num, 0)
            ...
            if result["success"]:
                ...
                if result.get("retryable", True) and current_retry < max_retries:
                    # Re-queue with jitter
                    with retry_lock:
                        retry_counts[page_num] = current_retry + 1
                    ...
                    logger.warning(
                        f"  Page {page_num} attempt {current_retry + 1}/{max_retries} failed: {result['error']}, re-queuing"
                    )
                else:
                    ...
                    logger.error(
                        f"  Page {page_num}: Failed after {current_retry + 1} attempts: {result['error']}"
                    )
