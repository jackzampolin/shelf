"""
OCRStageV2: Parallel OCR with unified page iteration.
...
class OCRStageV2(BaseStage):
    """
    OCRv2: Unified page iteration with parallel provider execution.
    ...
    def __init__(
        ...
        self.status_tracker = OCRStageV2Status(
            stage_name=self.name,
            provider_names=self.provider_ids
        )
        self.ocr_storage = OCRStageV2Storage(stage_name=self.name)
    ...
    def before(self, storage: BookStorage, checkpoint: CheckpointManager, logger: PipelineLogger):
        ...
        logger.info(f"OCRv2 with {len(self.providers)} providers:")
    ...
    def run(
        ...
        logger.info(f"OCR v2 Status: {status}")
