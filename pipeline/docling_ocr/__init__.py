...
class DoclingOcrStage(BaseStage):
    name = "docling-ocr"
    ...
    def default_kwargs(cls, **overrides):
        return {
            'max_workers': overrides.get('workers', 1),  # Sequential processing (Docling not thread-safe)
            'use_mlx': overrides.get('use_mlx', True)    # Default to MLX on macOS
        }
